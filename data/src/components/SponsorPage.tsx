...
import { UserProfile, AreaSponsor, SponsorInitiative, Community } from '../types';
...
interface SponsorPageProps {
  ...
  onCreateHelp?: (data: any) => Promise<void>;
...
export const SponsorPage: React.FC<SponsorPageProps> = ({ 
  ...
  const [isCreateSponsorHelpOpen, setIsCreateSponsorHelpOpen] = useState(false);
  
  // Create Sponsor Help Form State
  const [sponHelpTitle, setSponHelpTitle] = useState('');
  const [sponHelpDesc, setSponHelpDesc] = useState('');
  const [sponHelpCategory, setSponHelpCategory] = useState('Supporto Anziani');
  const [sponHelpBriko, setSponHelpBriko] = useState(100);
  const [sponHelpTargetCommIds, setSponHelpTargetCommIds] = useState<string[]>([]);
  const [isCreatingSponHelp, setIsCreatingSponHelp] = useState(false);
  ...
  const handleCreateSponsorHelpSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!onCreateHelp || !sponHelpTitle.trim() || !user) return;

    setIsCreatingSponHelp(true);
    try {
      await onCreateHelp({
        type: 'request',
        title: sponHelpTitle.trim(),
        description: sponHelpDesc.trim(),
        category: sponHelpCategory,
        creditsRequired: 0,
        isFree: true,
        valoreGentilezzaBriko: sponHelpBriko,
        targetCommunityIds: sponHelpTargetCommIds,
        sponsorId: 'spon-user-' + user.id,
        sponsorName: sponsorName || user.nickname,
      });
      setIsCreateSponsorHelpOpen(false);
      setSponHelpTitle('');
      setSponHelpDesc('');
      setSponHelpTargetCommIds([]);
      alert(`🎉 Richiesta di Gentilezza Sponsorizzata Pubblicata! I cittadini che ti aiuteranno riceveranno ${sponHelpBriko} BRIKO.`);
    } catch (err) {
      console.error('Error creating sponsor help:', err);
    } finally {
      setIsCreatingSponHelp(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8 animate-in fade-in duration-300">
      ...
      <div className="bg-gradient-to-r from-amber-500 via-amber-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5 text-center shrink-0 min-w-[240px]">
          ...
          <div className="mt-4 space-y-2">
            ...
            <button
              type="button"
              onClick={() => setIsCreateSponsorHelpOpen(true)}
              className="w-full bg-slate-900 text-white font-black py-2.5 px-3 rounded-xl text-xs transition-all hover:bg-slate-800 cursor-pointer shadow-sm flex items-center justify-center gap-1.5"
            >
              <Award className="w-4 h-4 text-amber-400" />
              <span>+ Chiedi Gentilezza Sponsorizzata</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {/* MODAL: Creare una Richiesta di Gentilezza Sponsorizzata */}
      {isCreateSponsorHelpOpen && (
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-100 animate-in fade-in zoom-in duration-200">
            <div className="bg-slate-900 p-5 text-white flex items-center justify-between">
              <h3 className="text-lg font-bold flex items-center gap-2">
                ...
                <span>Richiedi Gentilezza Sponsorizzata</span>
              </h3>
              <button onClick={() => setIsCreateSponsorHelpOpen(false)} className="text-white hover:opacity-80 p-1 cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>

            <form onSubmit={handleCreateSponsorHelpSubmit} className="p-6 space-y-4 text-xs">
              <div className="bg-amber-50 border border-amber-200 p-3 rounded-xl text-amber-950 leading-relaxed font-medium">
                <strong>Gentilezza Sponsorizzata:</strong> Questa richiesta apparirà nella bacheca principale. Chi risponderà riceverà i BRIKO che metti in palio.
              </div>

              <div>
                <label className="block font-bold text-slate-700 mb-1 uppercase tracking-wider">Cosa ti serve? (Titolo) *</label>
                <input
                  type="text"
                  value={sponHelpTitle}
                  onChange={(e) => setSponHelpTitle(e.target.value)}
                  placeholder="es. Consegna fiori per evento locale"
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1 uppercase tracking-wider">Descrizione Dettagliata *</label>
                <textarea
                  value={sponHelpDesc}
                  onChange={(e) => setSponHelpDesc(e.target.value)}
                  rows={3}
                  placeholder="Spiega come un cittadino può aiutarti..."
                  className="w-full px-3 py-2 rounded-xl border border-slate-200 focus:ring-2 focus:ring-amber-500 focus:outline-none font-medium"
                  required
                />
              </div>

              <div>
                <label className="block font-bold text-slate-700 mb-1 uppercase tracking-wider">Categoria</label>
                <select
                  value={sponHelpCategory}
                  onChange={(e) => setSponHelpCategory(e.target.value)}
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1 uppercase tracking-wider">Premio in BRIKO per chi aiuta *</label>
                <select
                  value={sponHelpBriko}
                  onChange={(e) => setSponHelpBriko(Number(e.target.value))}
              ...
              {communities.length > 0 && (
                <div>
                  <label className="block font-bold text-slate-700 mb-1 uppercase tracking-wider">Dedica a Comunità Specifiche (Opzionale)</label>
                  ...
                  <p className="text-[10px] text-slate-400 mt-1">Se non selezioni nulla, sarà visibile a tutti.</p>
                </div>
              )}

              <button
                type="submit"
                disabled={isCreatingSponHelp}
                className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 rounded-xl transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
              >
                {isCreatingSponHelp ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
                <span>Pubblica Richiesta Sponsorizzata</span>
              </button>
            </form>
          </div>
        </div>
